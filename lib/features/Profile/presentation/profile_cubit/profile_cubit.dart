...
class ProfileCubit extends Cubit<ProfileState> {
  ProfileCubit({required this.authRepository, required this.profileRepository}) : super(ProfileInitial());
  ...
  Future<void> getUserProfile() async {
    ...
    result.fold((failure) => emit(ProfileFailure(failure.error)), (user) => emit(ProfileSuccess(user)));
  }
  ...
  Future<void> changeUserPassword(ChangePasswordRequest request) async {
    ...
    result.fold((failure) => emit(ChangePasswordFailure(failure.error)), (result) => emit(ChangePasswordSuccess()));
  }
}
