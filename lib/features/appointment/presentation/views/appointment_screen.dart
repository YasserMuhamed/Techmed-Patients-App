...
import 'package:techmed/features/appointment/presentation/widgets/appointments_loading_cards.dart' show AppointmentLoadingCards;
    ...
    return Scaffold(
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(S.of(context).appointments, style: AppTextStyles.poppins18Bold(context)),
              26.verticalSpace,
              Expanded(
                child: BlocBuilder<AppointmentsCubit, AppointmentsState>(
                  buildWhen: (previous, current) {
                    return current is AppointmentsLoading || current is AppointmentsSuccess || current is AppointmentsFailure;
                  },
                  builder: (context, state) {
                    if (state is AppointmentsLoading) {
                      return const AppointmentLoadingCards();
                    } else if (state is AppointmentsSuccess) {
                      if (state.appointments.data == null || state.appointments.data!.isEmpty) {
                        return RefreshIndicator(
                          onRefresh: () => BlocProvider.of<AppointmentsCubit>(context).getAppointments(),
                          child: SingleChildScrollView(
                            physics: const AlwaysScrollableScrollPhysics(),
                            child: Column(
                              children: [
                                SizedBox(height: MediaQuery.of(context).size.height * 0.3),
                                Center(child: Text(S.of(context).no_appointments_found, style: AppTextStyles.poppins16Medium(context))),
                              ],
                            ),
                          ),
                        );
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return AppointmentCard(
                              appointmentDate: DateFormat('yyyy-MM-dd', "en-US").format(appointment.date!),
                              appointmentTime: appointment.time!,
                              doctorName: appointment.doctor!.name!,
                              hospitalName: appointment.hospital!.name!,
                              onTap: () {
                                context.pushNamed(AppRoutes.kAppointmentDetailsScreen, arguments: appointment.id).then((value) {
                                  if (value == true) {
      ...
      floatingActionButton: DecoratedBox(
        ...
        child: TextButton.icon(
          ...
          label: Text(S.of(context).add_appointment, style: AppTextStyles.poppins16Bold(context).copyWith(color: AppColors.white)),
        ),
      ),
    );
  }
