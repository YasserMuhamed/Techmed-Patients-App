...
class AppointmentsCubit extends Cubit<AppointmentsState> {
  ...
  Future<void> getAppointments() async {
    ...
    result.fold((failure) => emit(AppointmentsFailure(failure.error)), (appointments) => emit(AppointmentsSuccess(appointments)));
  }

  Future<void> getAppointmentDetails(int appointmentId) async {
    emit(AppointmentDetailsLoading());
    final result = await appointmentRepository.getSingleAppointment(appointmentId);
    result.fold((failure) => emit(AppointmentDetailsFailure(failure.error)), (appointment) => emit(AppointmentDetailsSuccess(appointment)));
  }

  Future<void> createAppointment(CreateAppointmentRequest appointmentData) async {
    emit(AppointmentCreateLoading());
    final result = await appointmentRepository.createAppointment(appointmentData);
    result.fold((failure) => emit(AppointmentCreateFailure(failure.error)), (_) => emit(AppointmentCreateSuccess()));
  }

  Future<void> cancelAppointment(int appointmentId) async {
    emit(AppointmentCancelLoading());
    final result = await appointmentRepository.cancelAppointment(appointmentId);
    result.fold((failure) => emit(AppointmentCancelFailure(failure.error)), (_) => emit(AppointmentCancelSuccess()));
  }

  Future<void> getAllHospitals() async {
    emit(HospitalsLoading());
    final result = await appointmentRepository.getHospitals();
    result.fold((failure) => emit(HospitalsFailure(failure.error)), (hospitals) => emit(HospitalsSuccess(hospitals)));
  }
}
